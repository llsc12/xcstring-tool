...
class FileLoadingModel {
	...
	var file: URL? {
		didSet {
			if let file {
				guard let file = loadFile(file) else { log("Error loading file"); return }
				self.newFileLoadedCallback?(file)
			} else {
				assert(localizationFile == nil, "Localization file should be nil before file url is set to nil")
			}
		}
	}

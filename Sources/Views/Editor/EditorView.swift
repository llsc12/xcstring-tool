			...
			VStack(alignment: .center) {
				...
				ForEach(
					...
					Button(" \(language) ") {
						...
						self.editorViewModel.selectedLanguage = language
					}
					.padding()
					.frame(maxWidth: .infinity)
					.background(
						editorViewModel.selectedLanguage == language ? Color.gray : .default
					)
				}
			}
			.frame(width: 25)
...
struct LocalizationEditor: View {
	...
	var initialValue: String {
		localization.localizations?[selectedLanguage]?.stringUnit?.value ?? editorViewModel.selectedKey ?? ""
	}
